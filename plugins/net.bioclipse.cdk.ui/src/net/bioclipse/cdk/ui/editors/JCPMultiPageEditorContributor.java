...
import org.openscience.cdk.applications.jchempaint.BCJCPPopupController;
...
import org.openscience.cdk.applications.jchempaint.MenuBarMaker;
...
//            super.setActiveEditor(part);
            if (((JCPMultiPageEditor)activeEditorPart).getJcpModel() != null) {
                registerModel(((JCPMultiPageEditor)activeEditorPart).getJcpModel());
                ((JCPMultiPageEditor)activeEditorPart).setContributor(this);
            }
        }
    }
    ...
    {
        if (model != null) {
            DrawingPanel drawingPanel = ((JCPMultiPageEditor)activeEditorPart).getDrawingPanel();
            ...
            SmilesParser sp=new SmilesParser(null);//TODO: no SmilesParser here please
            ...
            PopupController2D inputAdapter = new BCJCPPopupController((ChemModel) model.getChemModel(), model.getRendererModel(),model.getControllerModel(), null, null, ((JCPMultiPageEditor)activeEditorPart).getJcpComposite(),funcgroups);
            JCPBioclipseUndoRedoHandler undoRedoHandler=new JCPBioclipseUndoRedoHandler();
            undoRedoHandler.setDrawingPanel(((JCPMultiPageEditor)this.getActiveEditorPart()).getDrawingPanel());
            undoRedoHandler.setJcpm(model);
            undoRedoHandler.setUndoContext(((JCPMultiPageEditor)this.getActiveEditorPart()).getUndoContext());
    ...
    public void updateModel(IChemModel chemModel){
        ((PopupController2D)((JCPMultiPageEditor)activeEditorPart).getDrawingPanel().getKeyListeners()[0]).setChemModel(chemModel);
        ((JCPMultiPageEditor)activeEditorPart).getDrawingPanel().updateRingSetInRenderer();
    }
